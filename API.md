...
- **Кодировка**: UTF-8
- **Rate Limiting**: Не применяется (для учебного проекта)
...
В учебной версии rate limiting не применяется. В production рекомендуется добавить ограничения:

- Поисковые запросы: 100 запросов в минуту
- API для элементов: 1000 запросов в час
- Комментарии: 50 запросов в час

## Версионирование

API не имеет версионирования в URL. Изменения совместимы назад. Крупные изменения будут сопровождаться уведомлениями.
